import { CursorPick, ColorPicker } from "./colorPickerGUI.js";
import { Button, ButtonGroup, Checkbox, RadioGroup, Panel } from "../button.js";
import { drawRoundRect, drawLine, drawCircle } from "../tech.js";
import { CursorConnect, CLR_CON_TP, ColorCursor } from "./colorCore.js";
import { mmin, mmax, dist, arrDel, arrFind, arrRemAll, makeCopy } from "../math.js";



type CursPanObj={cpick:CursorPick, but:Checkbox, conns:{c:CursPanObj,c2:CursPanObj, type:number}[], crtInfo:{par?: CursorPick; type?: string; targ?: CursorPick; perc?: number;}}
type PaletteState={cupanos: {cpInd:number, crtInfo:{par?: number; type?: string; targ?: number; perc?: number;}, prop:{x:number,y:number,z:number,w:number,h:number} }[] };
//  conns:{cind:number,c2ind:number,type:number}[],
class ColorPalette{
    curspanos:CursPanObj[]=[];
    pan:Panel;
    cursorRG:RadioGroup= new RadioGroup();
    x2:number;y2:number;
    constructor(public gui:ButtonGroup, public picker:ColorPicker, public x:number=0,public y:number=0,public w:number=0,public h:number=0){
        let main=this;
		...
		this.pan.drawMe = function(ctx){
			...
			for(let cc of main.curspanos)
				for(let con of cc.conns)
					if(con.type<2)drawLine(ctx,cc.but.x,cc.but.y,con.c.but.x,con.c.but.y,
						(['#000000','#3377aa'])[con.type],con.type*2+1);
					else
						if(con.type==2)drawLine(ctx,cc.but.x,cc.but.y,(con.c.but.x+con.c2.but.x)/2,(con.c.but.y+con.c2.but.y)/2,
						(['#000000','#335588'])[con.type-2],con.type*1+1);
				
		}
		if(w*h>0){this.initPos(x,y,w,h);this.initDraw();}
	}
    initPos(x,y,w,h){
    ...
    addPanObj(cur:CursorPick,info: { par?: CursorPick; type?: string; targ?: CursorPick; perc?: number; } = {}){
		let cpo=this.crtPanObj(cur,info);
		this.curspanos.push(cpo);
        this.cursorRG.add(cpo.but);
		return cpo;
	}
    crtPanObj(cur:CursorPick,info: { par?: CursorPick; type?: string; targ?: CursorPick; perc?: number; } = {}):CursPanObj{
        ...
        let cobj:CursPanObj = {cpick:cur, but:b, conns:[], crtInfo:info}, main=this;
        switch(type){
			case "simple":
        ...
        let msMoveFun = function(mx,my,rmb){if(!rmb){
						this.props.moveMe.call(this,mmin(main.x2-this.w/2,mmax(main.x+this.w/2,mx-this.props.dragx)),
						                            mmin(main.y2-this.h/2,mmax(main.y+this.h/2,my-this.props.dragy)));
                    }}
		switch(type){
			case "twin":
				...
				b.msmove = par.panobj.but.msmove = function(mx,my,rmb){
					...
					if(d>max){
						msMoveFun.call(this.props.twin,
							this.props.twin.x+this.props.twin.props.dragx+(this.x-x)*(d-max)/d,
							this.props.twin.y+this.props.twin.props.dragy+(this.y-y)*(d-max)/d,false);
			...
			case "range":
				...
				b.props.perc=perc;
				b.props.renewPos=() =>{
						b.props.moveMe.call(b,(b.props.par1.x)*b.props.perc+(b.props.par2.x)*(1-b.props.perc),
						                      (b.props.par1.y)*b.props.perc+(b.props.par2.y)*(1-b.props.perc));
						if(b.par==null){
		...
		b.props.dragx=0;b.props.dragy=0;;
		b.msdown = function(mx,my,rmb){
			if(rmb){
				...
				if(picker.buts.crtrange.checked && this.props.cursor!=picker.selectC.c){
					...
					for(let i=0;i<N;i++)
                        picker.createCursor(1,1,1,
                            {par:this.props.cursor,type:"range",targ:targ, perc:(i+1)/(N+1)});
				}
				picker.selectCursor(this.props.cursor);
				this.props.dragx=mx-this.x;this.props.dragy=my-this.y;
			}
		}
		...
		b.cntrx=0;b.cntry=0;	//shift center for proper inMask detection
    ...
    deletePanObj(cur:CursPanObj){
        ...
        // }
        for(let c of this.curspanos)arrRemAll(c.conns,(a)=>(this.curspanos.indexOf(a.c)<0));
	}
	
	createPaletteStateObj(cpicks:CursorPick[]):PaletteState{
		let palst:PaletteState = {cupanos:[]};
	...
	loadPaletteStateObj(pstate:PaletteState, cpicks:CursorPick[]):CursPanObj[]{//
		let curspanos:CursPanObj[]=[];
		for(let cps of pstate.cupanos){
			let cp:CursPanObj=this.crtPanObj( cpicks[cps.cpInd],  {par:cpicks[cps.crtInfo.par],targ:cpicks[cps.crtInfo.targ],type:cps.crtInfo.type,perc:cps.crtInfo.perc});
			curspanos.push(cp);
...
export {CursPanObj, ColorPalette as CursorPanel, PaletteState}