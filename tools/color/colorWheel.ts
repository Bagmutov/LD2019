import { mmin, mmax, mabs, pow2, dist, arrFindMin, isNum, arrFind } from "../math.js";
import { toClr, getAng, rgbAdd, COLOR, RGB, ColorCursor, HSV, HSVtoRGB } from "./colorCore.js";
import { Button, Checkbox, Slider, RadioGroup, ButtonGroup } from "../button.js";
import { ColorPicker, CursorPick } from "./colorPickerGUI.js";
import { drawCircle } from "../tech.js";

type WheelFun = (a:number, r:number)=>RGB;
type CursObj = {curs:ColorCursor,dx:number,dy:number};

class ColorWheelEdi{
    x:number;y:number;w:number;//h:number;
    whBut:Button;
    stripMode:boolean=false;
    cntrx:number; cntry:number;
    attrAngs:number[]=[0];
    wheelFun:WheelFun = ColorPicker.wheels[0];
    prism:Prism = ColorPicker.prisms[0];
    cursObjs:CursObj[]=[];
    private lastDrawWidth:number = 0;
    constructor(public gui:ButtonGroup, public picker:ColorPicker){
        let edi=this;
        ...
        this.whBut.drawMe=function(ctx){
            ...
            for(var c of edi.cursObjs){
                ...
                else drawCircle(ctx,edi.cntrx+c.dx*edi.w,edi.cntry+c.dy*edi.w,3,'#000000',(picker.selectC.c && c.curs==picker.selectC.c.curs)?3:1);
            }
            ctx.beginPath();
            ctx.lineWidth=.2;
            if(edi.stripMode){ctx.moveTo(this.x,this.y+edi.cursObjs[0].curs.pos.s*this.h);ctx.lineTo(this.x+this.w,this.y+edi.cursObjs[0].curs.pos.s*this.h);}
            else ctx.arc(edi.cntrx,edi.cntry,edi.cursObjs[0].curs.pos.s*this.w/2,0,6.29);
            ctx.stroke();
        };
        this.whBut.msdown=function(mx,my,rmb){
            // if(mx>x && mx<x+w && my>y && my<y+h){
                ...
                let min = arrFindMin<CursObj>(edi.cursObjs, (c)=>(dist(x-c.dx,y-c.dy)));
                // var md = 2,dd,mc, 
                // for(var c of this.edi.cursObjs){dd=dist(x-c.dx,y-c.dy);if(dd<md){md=dd;mc=c;}};
                if(min.d<.1){picker.selectCursorInd(arrFind(picker.curspicks,c=>c.curs==min.o.curs));}//min.o.but.mmsdown(0,0,rmb);min.o.but.mmsclick(0,0,rmb);
    ...
    initPos(x,y,w){
        ...
        this.cntrx=x+w/2; this.cntry=y+w/2;
    }
    initDraw(){
    ...
    processInput(dx,dy){
		dx=dx/this.w;dy=dy/this.w;
		var r=(this.stripMode)?dy+.5:dist(dx,dy)*2;
        if(!this.stripMode) {if( r>.97){dx=(dx)*.97/r;dy=(dy)*.97/r; r=1;}}
        else {dx=Math.max(Math.min(dx,.5),-.5);dy=Math.max(Math.min(dy,.5),-.5);r=dy+.5;}
    ...
    addCObj(c:CursorPick){
        this.cursObjs.push({curs:c.curs, dx:0, dy:0});
        this.moveCursors();
    }
    delCObj(c:CursorPick){
        let i = arrFind<CursObj>(this.cursObjs,(o)=>(o.curs==c.curs));
	...
	redraw(){
		...
		if(this.stripMode)drawColorStrip(this.whBut.ctx,0,0,this.w,this.w,this.wheelFun,this.prism);
        else {
    ...
    whToStrip(){
        for(var c of this.cursObjs){
            c.dx=c.curs.pos.h-.5; c.dy=c.curs.pos.s-.5;
        }
    }
    stripToWh(){
        for(var c of this.cursObjs){
            c.dx=Math.cos(c.curs.pos.h*6.28)*c.curs.pos.s/2; c.dy=Math.sin(c.curs.pos.h*6.28)*c.curs.pos.s/2;
        }
    }
...
function drawColorStrip(ctx,x,y,w,h,whFun:WheelFun, prism:Prism=null){
	var a;
	for(var i=x;i<x+w;i++)
		for(var j=y;j<y+h;j++){
			var dx=i-x, dy=j-y;
...
function drawColorWheel(ctx,x,y,r,whFun:WheelFun, prism:Prism){
	var a;
	for(var i=-r;i<r;i++)
		for(var j=-r;j<r;j++){
			var d=dist(i,j)/r;
...
const WHEEL = {
    ...
    spiral : function(ang,rad){
        ...
        var r=mmin(0xff,2*mmax(0xff-mabs(ang>OL?ang-6.28:ang)*0xff/OL,0)),
            g=mmin(0xff,2*mmax(0xff-mabs(2.1-ang)*0xff/OL,0)),
            b=mmin(0xff,2*mmax(0xff-(mabs((ang<OL?ang+6.28:ang)-4.2))*0xff/OL,0));
    ...
    triangles : function(ang,rad){
        ang=(ang+6.28)%6.28;
        var r=2*Math.max(0xff-Math.abs(ang>OL?ang-6.28:ang)*0xff/OL,0),
            g=2*Math.max(0xff-Math.abs(2.1-ang)*0xff/OL,0),
            b=2*Math.max(0xff-(Math.abs((ang<OL?ang+6.28:ang)-4.2))*0xff/OL,0);
...
class Prism{
    params:{h:number, k:number, c:number}[]=[];
    constructor(public pts: { x: number, y:number}[], reverse=false){
        var lst=[],k;
    ...
    apply(ang:number):number{
        var i=0;
        while(ang>this.params[i].h)i++;
