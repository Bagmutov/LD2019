import { mmin, mmax, arrFind } from "../math.js";
import { ButtonGroup, Checkbox, Button, Switcher, Slider, MOUSE, NumSlider, SHAPES, Panel } from "../button.js";
import { WHEEL, PRISM, ColorWheelEdi, drawColorStrip, } from "./colorWheel.js";
import { ColorCursor, HSV, HSVtoRGB, hsvSet, RGB, createColorStateObj, cursorsFromColorState } from "./colorCore.js";
import { drawRoundRect, drawShape, saveFile, loadJSON } from "../tech.js";
import { CursorPanel } from "./colorPan.js";
class ColorPicker {
    constructor({ x = 0, y = 0, w = 0, h = 0, colorN = 0, clrs = null } = {}) {
        this.gui = new ButtonGroup(1);
        this.colorOutputs = [];
        this.curspicks = [];
        this.selectC = { c: null, i: -1 };
        this.colorUser = null;
        this.colors = [];
        ...
        let picker = this;
        this.buts = {
            back: new Panel(this.gui, x, y, w, h, 2, 0, 0),
            ...
            valSlider: new Slider({ par: this.gui, val: 1,
                valFun: (v) => { if (picker.selectC.c)
                    picker.selectC.c.curs.changePos(undefined, undefined, v, true); picker.renewCursClrs(); } }),
        ...
        this.buts.crtcurs.msclick = (mx, my, rmb) => { if (picker.selectC.c)
            picker.createCursor(1, 1, 1, { par: picker.selectC.c, type: "simple" }); };
        this.buts.crttwins.msclick = (mx, my, rmb) => {
            if (!picker.selectC.c)
                return;
    ...
    initDraw(clrs) {
        ...
        this.buts.saveout.drawAllCanv((self, ctx, ind) => {
            drawRoundRect(ctx, 0, 0, self.crad, self.w, self.h, clrs.butbk[ind]);
            drawShape(ctx, self.w, self.h / 10, SHAPES.rect, clrs.butpic[ind], .1, 5, .8, -.1);
            drawShape(ctx, self.w, self.h / 10, SHAPES.rect, clrs.butpic[ind], .1, 3, .8, -.11);
            drawShape(ctx, self.w * .9, self.h / 10, SHAPES.rect, clrs.butpic[ind], .15, 7, .8, -.1);
        }, true);
        this.edi.initDraw();
    ...
    createCursor(h, s, v, other = {}) {
        var c = { curs: new ColorCursor(new HSV(h, s, v)), color: '#ffffff', panobj: null };
    ...
    setSelCursor(ang, rad) {
        if (!this.selectC.c)
            return;
        this.selectC.c.curs.changePos(ang / 6.28, rad, undefined, true);
        this.renewCursClrs();
    }
    selectCursor(c) { this.selectCursorInd(this.curspicks.indexOf(c)); }
    selectCursorInd(ind) { this.selectC.i = ind; this.selectC.c = this.curspicks[ind]; this.buts.valSlider.val = this.selectC.c.curs.pos.v; }
    deleteCursor(c, ci = this.curspicks.indexOf(c), deleteFirst = false) {
        if (ci == 0 && !deleteFirst)
            return;
        if (ci < 0)
            throw new Error('delete cursor err');
        let delcs = [], ths = this;
        ...
        function delCurs(c) {
            ...
            if (ths.selectC.i > ci)
                ths.selectC.i--;
            if (ths.selectC.i == ci)
                ths.selectC = { c: null, i: -1 }; //ths.selectCursorInd(0);
            ths.edi.delCObj(c);
            ths.colorPan.deletePanObj(c.panobj);
            for (let o of ths.colorOutputs) {
                if (o.props.cursor == c)
                    o.props.cursor = ths.curspicks[0];
                if (o.props.prev == c)
                    o.props.prev = ths.curspicks[0];
            }
        }
    }
    renewCursClrs() {
        ...
        if (this.colorUser)
            this.colorUser.changeColors(this.getColors());
    }
    getColors() {
        for (var i = 0; i < this.colorOutputs.length; i++)
            this.colors[i] = this.colorOutputs[i].props.cursor.color;
        return this.colors;
    }
    addColorUser(user) {
        this.colorUser = user;
    ...
    loadFromEditorState(es) {
        this.deleteCursor(this.curspicks[0], 0, true); //Delete all cursors!
        // this.edi.delCObj(cp);
        let curspicks = [];
        let ccurs = cursorsFromColorState(es.cs);
        for (let cc of ccurs)
            curspicks.push({ curs: cc, color: cc.getClr(), panobj: null });
        let cursopanos = this.colorPan.loadPaletteStateObj(es.ps, curspicks);
    ...
    activate(val) { this.gui.active = val; }
    ;
}
ColorPicker.prisms = [PRISM.lineRev, PRISM.rybAdobe, PRISM.rybRev]; //not in use for now
ColorPicker.wheels = [WHEEL.basic, WHEEL.constIntens, WHEEL.spiral, WHEEL.triangles]; //not in use for now
export { ColorPicker };
//# sourceMappingURL=colorPickerGUI.js.map