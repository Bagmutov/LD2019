import { resize, drawCircle } from "./tools/tech.js";
...
class ClrControl {
    constructor(picker) {
        ...
        this.tv = 1;
        this.numOfColors = 8;
        this.labels = ["back1", "back2", "back3", "text", "act_text", "partcls1", "partcls2", "partcls3"];
        this.colors = { text: '#333333', textact: '#9966aa', back: ['#ffffff', '#222222', '#444444'], ptcls: ['#ffffff', '#999999', '#aaaaaa'] };
    }
    changeColors(clrs) {
    ...
    step() {
        if (this.th) {
            this.h += sign(this.th - this.h) * .001;
            if (mabs(this.th - this.h) < .01)
                this.th = null;
        }
        if (this.ts) {
            this.s += sign(this.ts - this.s) * .001;
            if (mabs(this.ts - this.s) < .01)
                this.ts = null;
        }
        if (this.tv) {
            this.v += sign(this.tv - this.v) * .001;
            if (mabs(this.tv - this.v) < .01)
                this.tv = null;
        }
...
export let backbackCan = document.getElementById('canbackback'), ctxbb = backbackCan.getContext('2d');
export let backCan = document.getElementById('canback'), ctxb = backCan.getContext('2d');
let backStyle = "stars", backInit = true, arr = [], arr2 = [], backobj = {};
resize(fMain);
resize(backCan);
resize(backbackCan);
